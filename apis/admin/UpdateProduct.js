...
async function UpdateProduct(req, res) {
  try {
    ...
    if (!id || !ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Valid product ID is required" });
    ...
    if (stock !== undefined) { updateFields.stock = parseInt(stock); updateFields.status = parseInt(stock) > 0 ? "Available" : "Out_of_Stock"; }
